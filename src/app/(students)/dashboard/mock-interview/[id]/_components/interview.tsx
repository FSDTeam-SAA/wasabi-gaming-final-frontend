...
const Interview: React.FC<InterviewProps> = ({ sessionData, onBack }) => {
  ...
  // ✅ ONLY CHANGE: Max recording time = 20 minutes
  const MAX_RECORDING_TIME = 20 * 60; // 1200 seconds
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      // ✅ ONLY CHANGE: Start recording timer up to 20 minutes
      recordingTimerRef.current = setInterval(() => {
        setRecordingTime((prev) => {
      ...
      <div className="flex flex-col gap-6 md:flex-row">
        ...
        <div className="lg:w-[60%] border-[3px] border-[#FAFF00] rounded-2xl p-6 flex flex-col justify-between relative">
          ...
          <div className="flex items-center gap-4 mt-6">
            ...
            <div>
              ...
              <p className="text-sm italic text-gray-600">
                {isRecording
                  ? `${formatTime(recordingTime)} of ${formatTime(
                      MAX_RECORDING_TIME,
                    )}`
                  : isVideoRecorded
                    ? "Your response has been recorded"
        ...
        <div className="flex-1 border-[3px] border-[#FAFF00] rounded-2xl p-8 flex flex-col justify-between">
                  ...
                  <div
                    ...
                    style={{
                      width:
                        questions.length > 0
                          ? `${
                              (answers.filter((a) => a.submitted).length /
                                questions.length) *
                              100
                            }%`
                          : "0%",
          ...
          <div className="mt-8 text-center">
            ...
            <p className="mt-3 text-sm font-bold opacity-80">
              {isRecording
                ? `Recording... ${formatTime(recordingTime)} / ${formatTime(
                    MAX_RECORDING_TIME,
                  )}`
                : isVideoRecorded
                  ? "You can download or retake your recording"
