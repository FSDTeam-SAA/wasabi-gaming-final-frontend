...
import { CheckCircle2, Lightbulb, PlayCircle, ChevronRight, Trophy } from "lucide-react";
  ...
  return (
    <div className="flex items-center justify-between p-6 bg-[#FFFFF0] rounded-xl border border-gray-100 shadow-sm">
      ...
      <div className="relative flex items-center justify-center">
        ...
        <span className="absolute text-sm font-semibold text-gray-600">{value}%</span>
      </div>
    </div>
  );
};
    ...
    <div className="space-y-8">
      ...
      <div className="bg-[#1A1A1A] text-white p-4 rounded-xl flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] uppercase tracking-widest">Interview Crushed!</p>
            <h2 className="text-xl font-bold">{averageScores.interview_crushed}/100</h2>
          </div>
        </div>
      </div>
      ...
      <div className="p-8 border border-gray-200 rounded-2xl">
        <div className="flex items-center gap-2 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold leading-tight text-gray-900">Score Breakdown</h2>
            <p className="text-xs font-medium text-gray-500">Mock Interview: Skills & Performance Review</p>
          </div>
        </div>

        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <RadialProgress label="Communication & Clarity" value={averageScores.communication_and_clarity} />
          <RadialProgress label="Commercial Awareness" value={averageScores.commercial_awareness} />
          <RadialProgress label="Problem Solving" value={averageScores.problem_solving} />
          <RadialProgress label="Professionalism & Presence" value={averageScores.professionalism_and_presence} />
        </div>
      </div>
        ...
        <div className="bg-[#F0F9F4] border border-[#B7E4C7] rounded-3xl p-6">
          ...
          <ul className="space-y-4">
            {feedback.strength.map((item: string, idx: number) => (
              <li key={idx} className="flex gap-3 text-sm text-[#2D6A4F] leading-relaxed">
        ...
        <div className="bg-[#FFF5F2] border border-[#FAD2C0] rounded-3xl p-6">
          ...
          <ul className="space-y-4">
            {feedback.areas_for_improvement.map((item: string, idx: number) => (
              <li key={idx} className="flex gap-3 text-sm text-[#94412D] leading-relaxed">
      ...
      <div className="bg-gradient-to-r from-[#E6E905] to-[#8C931B] rounded-3xl p-12 text-center space-y-4 shadow-lg">
        ...
        <h2 className="text-3xl font-black text-gray-900">Ready to See Your Performance?</h2>
        <p className="max-w-lg mx-auto text-sm font-medium text-gray-800">
          Watch your recorded responses, get detailed feedback on each question and see analysis of your body language and communication
        </p>
