...
const SettingHaders = ({ setIsLogoutModalOpen }: { setIsLogoutModalOpen: (isOpen: boolean) => void }) => {
        ...
        <div className="flex justify-between items-center">
            ...
            <div className="flex flex-col items-end gap-2">
                <button className="bg-gradient-to-r from-[#F3E8FF] to-[#DBEAFE] flex items-center justify-center  text-[#8200DB] px-4 py-1 rounded-full text-sm font-medium">
                    <Sparkles className="w-4 h-4 mr-2" /> {data?.data?.subscription?.name || "Free Plan"}
                </button>
                <button
