...
function ShowOpenPosition({
  ...
  // Track which jobs user has applied to in this session
  ...
  const handleApply = async (position: OpenPosition) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Get redirect URL from response (try multiple possible paths)
        const redirectUrl =
          result.data?.url ||
          result.data?.job?.url ||
      ...
      <div className="space-y-4">
        {openPositions.map((position) => {
          ...
          return (
            ...
            >
              <div className="mb-4">
                <h3 className="text-lg font-normal text-[#1E1E1E] mb-3">{position.title}</h3>
                ...
                <div className="flex justify-between items-center gap-6 mb-10 text-sm text-gray-600 flex-wrap">
                  <div className="flex items-center gap-1.5">
                    <svg className="w-4 h-4 text-[#DF0000]" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
                    </svg>
                    <span className="font-normal text-[#364153]">{position.location || 'Worldwide'}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-medium">{position.salaryRange || '£80,000 - £120,000'}</span>
                  </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`
                    ...
                    transition-colors text-sm flex items-center gap-2 min-w-[140px] justify-center
                    ${
                      hasApplied
                ...
                <button
                  ...
                  className="border-2 flex items-center gap-2 border-[#0000001A] hover:border-gray-400 text-[#1E1E1E] font-medium px-6 h-[44px] rounded-[14px] transition-colors text-sm"
                >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between rounded-t-2xl">
              <h2 className="text-2xl font-medium text-[#1E1E1E]">Job Details</h2>
              <button
            ...
            <div className="p-6 space-y-6">
              <div>
                <h3 className="text-2xl font-medium text-[#1E1E1E] mb-2">{selectedPosition.title}</h3>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <span
                  className={`px-4 py-2 rounded-full text-sm font-medium ${
                    selectedPosition.jobStatus === 'Open' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                  }`}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 rounded-xl p-6">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-[#1E1E1E]">{selectedPosition.jobId}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="font-medium text-[#1E1E1E]">{selectedPosition.startDate}</p>
                  </div>

                  <div>
                    <p className="text-sm text-gray-500 mb-1">Application Deadline</p>
                    <p className="font-medium text-red-600">{selectedPosition.applicationDeadline}</p>
                  </div>

                  <div>
                    <p className="text-sm text-gray-500 mb-1">Posted By</p>
                    <p className="font-medium text-[#1E1E1E]">{selectedPosition.postedBy}</p>
                  </div>
                </div>
              </div>
